...
def visualise_coverage_3D(cities : list[tuple[str, float, float]] | pd.DataFrame,
    ...
    if isinstance(cities, pd.DataFrame):
        ...
        covered_locations = np.array([cities[id] for id in covered_ids])
        city_locations = cities
    ...
    ax.scatter(city_locations[0], city_locations[1], city_locations[2], s=3, zorder=2, color="blue")

    covered_locations = covered_locations.T
    ax.scatter(covered_locations[0], covered_locations[1], covered_locations[2], s=3, color="green", zorder=3)
    ...
    if len(satellites) > 0:
        ...
        satellite_locations = satellite_locations.T
